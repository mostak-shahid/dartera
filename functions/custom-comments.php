		...
		if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type) { 
			...
			echo '<div class="media comment_section">';
				echo '<div class="media-body">';
					_e('Pingback:', 'dartera');
					comment_author_link(); 
					edit_comment_link(__('Edit', 'dartera'), '<span class="edit-link">', '</span>');
				echo '</div>';
			echo '</div>';
		} else {
			echo '<li id="comment-';
				...
				echo '<div class="comment-content media-body">';
					echo '<div class="well">';
						echo '<div class="comment-author vcard">';
							...
							// comment author says
							//printf(/*__('%s <span class="says">says:</span>', 'dartera'),*/ 
//                                sprintf('<span class="comment-author">%s</span>&nbsp; ', get_comment_author_link());
							...
							//edit_comment_link('<span class="fa fa-pencil-square-o "></span>' . __('Edit Comment', 'dartera'), '<span class="edit-link">', '</span>');
							...
							if ('0' == $comment->comment_approved) {
								echo '<div class="comment-awaiting-moderation text-warning"> <span class="glyphicon glyphicon-info-sign"></span> ';
									_e('Your comment is awaiting moderation.', 'dartera');
								echo '</div>';
							} //endif;
...
//								'reply_text' => 'Reply' . __('', 'dartera'),
//								'login_text' => '<span class="fa fa-reply"></span> ' . __('Log in to Reply', 'dartera')
//							)));
							// end reply link
					echo '</div><!-- .well -->';
...
add_filter('comment_form_defaults', 'dartera_comment_defaults');
function dartera_comment_defaults($defaults) {
	$defaults['comment_notes_before'] = '';
	$defaults['title_reply'] = 'Leave a comment';
	$defaults['title_reply_to'] = 'Leave a comment %s';
	$defaults['label_submit']  = __( 'Submit Comment', 'dartera' );
	return $defaults;
}
